...
//$query_udemo = "select * from ".tbl_product." where (status = 1 or status = 0) $get_price $owndata $hidedata and hide_id not like '%".$_SESSION['LOGINDATA']['USERID']."%' $lovedata  ORDER BY proid DESC";

//echo $select_products =  $query_demo." ".UNION." ".$query_udemo;
$view_pro = $db->get_row($query_demo,ARRAY_A);

$sub_categorys = mysql_real_escape_string(stripslashes(trim($view_pro['sub_category'])));
/***********************GET NEXT PRODUCT********************************/
 $nxtsql = "SELECT proid FROM gift_product WHERE proid>{$view_pro['proid']} and sub_category = '".$sub_categorys."' $get_data $get_price $get_occassion_data and (status = 1 or status = 0) $owndata $hidedata and hide_id not like '%".$_SESSION['LOGINDATA']['USERID']."%'";
 if(mysql_num_rows($chk_users) > 0) {
 $nxtsqls = "UNION SELECT proid FROM ".tbl_product." WHERE proid>{$view_pro['proid']} and sub_category = '".$sub_categorys."' $get_price and (status = 1 or status = 0) $owndata $hidedata and hide_id not like '%".$_SESSION['LOGINDATA']['USERID']."%' $lovedata";
 }
...
/***********************GET PREVIOUS PRODUCT********************************/
 $prevsql = "SELECT proid FROM gift_product WHERE proid<{$view_pro['proid']} and sub_category = '".$sub_categorys."' $get_data $get_price $get_occassion_data and (status = 1 or status = 0) $owndata $hidedata and hide_id not like '%".$_SESSION['LOGINDATA']['USERID']."%'";
 if(mysql_num_rows($chk_users) > 0) {
 $prevsqls = "UNION SELECT proid FROM ".tbl_product." WHERE proid<{$view_pro['proid']} and sub_category = '".$sub_categorys."' $get_price and (status = 1 or status = 0) $owndata $hidedata and hide_id not like '%".$_SESSION['LOGINDATA']['USERID']."%' $lovedata";
}
