<?php
$category = mysql_real_escape_string(stripslashes(trim(str_replace("_"," ",$_GET['s']))));
$query = "select * from ".tbl_product." where (sub_category like '%".$category."%') and (status = 1 or status = 0) and hide_id not like '%".$_SESSION['LOGINDATA']['USERID']."%'";
$get_pro = $db->get_results($query,ARRAY_A);

$view_cat = "select * from ".tbl_category." where status = 1 and cat_name like '%".$category."%' and p_catid != '0'";
...
$query = "select * from ".tbl_product." where sub_category like '%".mysql_real_escape_string(stripslashes(trim($child_cat['cat_name'])))."%' and (status = 1 or status = 0) and hide_id not like '%".$_SESSION['LOGINDATA']['USERID']."%'";
 $view_pro = $db->get_row($query,ARRAY_A);
 
/***********************GET NEXT PRODUCT********************************/
 $sql = "SELECT proid FROM gift_product WHERE proid>{$view_pro['proid']}  and category = '".mysql_real_escape_string(stripslashes(trim($view_pro['category'])))."' and sub_category like '%".mysql_real_escape_string(stripslashes(trim($view_pro['sub_category'])))."%' and (status = 1 or status = 0) and hide_id not like '%".$_SESSION['LOGINDATA']['USERID']."%' ORDER BY proid LIMIT 1";
...
?>
	...
	<div role="main" class="ui-content jqm-content jqm-content-c product_bar">
		<div class="product_title"><span><?php echo ucfirst(stripslashes($parent_cat['cat_name']));?>:</span> <?php echo ucfirst(stripslashes($child_cat['cat_name']));?> <img src="<?php echo ru_resource;?>images/arrow_b.jpg" alt="Arrow" /></div>
			...
			<div class="viewport">
				...
				<?php
					if($get_pro)
					 ...
					 {
				?>
					<?php /*?><div class="list" id="<?php echo $products['proid']; ?>"><?php */?>
					<div class="list">
						<a href="<?php echo ru;?>product_detail/<?php echo $products['proid'];?>" data-ajax="false"><img src="<?php  get_image($products['image_code']);?>" alt="<?php echo substr($products['pro_name'],0,20);?>"></a>
