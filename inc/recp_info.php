<?php
$recp_info = "select * from ".tbl_recipient." where recipit_id = '".$_SESSION['recipit_id']['New']."' and userId = '".$_SESSION['LOGINDATA']['USERID']."'";
$view_info = $db->get_row($recp_info,ARRAY_A);
?>
	...
	<form id="form_rec" method="post" action="<?php echo ru;?>process/process_delivery.php">
		<div class="ui-field-contain field"  style="display:none">
		<?php if($_SESSION['biz_rec_err']['Recp_edit']) { ?>
		<span style="color:#FF0000; font-weight:bold; float:none"><?php echo $_SESSION['biz_rec_err']['Recp_edit'];?></span>
		<?php } ?>
		...
		?>
			...
			<span style="color:#FF0000; font-weight:bold; float:none"><?php echo $_SESSION['biz_rec_err']['cash_amount'];?></span>
		</div>
		<h3 class="recip delivery">Edit Recipient Info</h3>
		...
		<input name="occassionid" id="occassionid" value="<?php echo $view_info['ocassion'];?>" type="hidden">
		<input name="SaveRecp" id="SaveRecp" value="1" type="hidden">
		<div class="ui-field-contain field">
			...
			<span style="color:#FF0000; font-weight:bold; float:none"><?php echo $_SESSION['biz_rec_err']['first_name'];?></span>
		</div>
		<div class="ui-field-contain field">
			...
			<span style="color:#FF0000; font-weight:bold; float:none"><?php echo $_SESSION['biz_rec_err']['last_name'];?></span>
		</div>
		<div class="ui-field-contain field">
			...
			<span style="color:#FF0000; font-weight:bold; float:none"><?php echo $_SESSION['biz_rec_err']['email'];?></span>
		</div>
		<a href="javascript:;" onclick="Recipitforms()" class="ui-btn ui-btn-c">Save and Continue</a>
		</form>
	</div><!-- /content -->
	
<script language="javascript">
	function Recipitforms(){
		document.getElementById("form_rec").submit()	
	}
</script>	
<?php
