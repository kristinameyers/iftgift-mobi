<?php
 ...
 $mor_query = "select * from ".tbl_product." where (sub_category like '%".mysql_real_escape_string(stripslashes(trim($view_pro['sub_category'])))."%') and (status = 1 or status = 0) and hide_id not like '%".$_SESSION['LOGINDATA']['USERID']."%'";
$get_pro = $db->get_results($mor_query,ARRAY_A);
 }
/***********************GET NEXT PRODUCT********************************/
...
?>
	...
	<div role="main" class="ui-content jqm-content jqm-content-c product_bar">
		<div class="product_title"><span><?php echo ucfirst(stripslashes($view_pro['category']));?>:</span> <?php echo ucfirst(stripslashes($view_pro['sub_category']));?> <img src="<?php echo ru_resource;?>images/arrow_b.jpg" alt="Arrow" /></div>
